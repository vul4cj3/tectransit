...
import {
  ShippingMInfo, ShippingHInfo, ShippingDInfo, DeclarantMemInfo, IDImgList,
  IDFileList, TransferMInfo, TransferHInfo, TransferDInfo
} from 'src/app/_Helper/models';
...
export class ShippingEditComponent implements OnInit {
  ...
  private data2Url = 'GetACShippingData';
  private saveUrl = 'SaveTransferDecData';
  private save2Url = 'SaveShippingDecData';
  ...
  masterData: TransferMInfo;
  headerData: TransferHInfo[];
  detailData: TransferDInfo[];
  declarData: DeclarantMemInfo[];

  masterData2: ShippingMInfo;
  headerData2: ShippingHInfo[];
  detailData2: ShippingDInfo[];
  ...
  getData() {
    let page = this.baseUrl + this.data2Url;
    if (this.shippingType === 't1' || this.shippingType === 't2') {
      page = this.baseUrl + this.dataUrl;
    }

    this.commonservice.getData(this.dataID, page)
      .subscribe(
        data => {
          if (data.status === '0') {
            if (this.shippingType === 't1' || this.shippingType === 't2') {
              ...
              this.masterData2 = data.rowM;
              this.headerData2 = data.rowH;
              this.detailData2 = data.rowD;
  ...
  chgDec(type) {
    if (type === 'm') {
      ...
      // read declarant data
      this.dataForm.controls.idm.setValue(this.masterData.id);
      const control = this.dataForm.controls.decform as FormArray;
    ...
    const promise1 = new Promise((resolve, reject) => {
      if (this.fileList.length > 0) {
        ...
        for (let i = 0; i < this.fileList.length; i++) {
          ...
          if (i === 0) {
            formdata2.append('TYPE', 'shipping');
          }
        }
  ...
  save() {
    let page = this.baseUrl + this.save2Url;
    if (this.shippingType === 't1' || this.shippingType === 't2') {
      page = this.baseUrl + this.saveUrl;
    }

    this.commonservice.editNDelData(this.dataForm.value, this.iddellist, page)
      .subscribe(data => {
        if (data.status === '0') {
          this.isModify = false;
          this.resetForm();
